...
def run_quarter(quarter_num):
    ...
    st.title(f"📊 Simulador de Cenários de Negócio - Trimestre {quarter_num}")
    ...
    st.write(f"Lucro Líquido: {net_profit:,.0f}")
    ...
    st.write(f"Rácio de Lucro Líquido: {net_profit_ratio:.2%}")
    st.write(f"ROI: {roi:.2%}")
    st.write(f"Rácio de Liquidez Corrente: {current_ratio:.2f}")
    st.write(f"Dias de Clientes: {debtors_days:.2f}")
